...
<body>
  ...
  <script>
    ...
    $('add-camera').onclick = async () => { try { const storeId=requireStore(), camera=state.selectedCamera, name=$('camera-name').value.trim(); if(!camera || !name) throw new Error('Selecciona una cámara y define su nombre.'); setStatus('scan-status','Capturando miniatura final…'); await request(`/stores/${storeId}/cameras`,{method:'POST',body:JSON.stringify({name,channel:camera.channel,location:'',is_active:true,collection_enabled:false})}); setStatus('scan-status','Cámara agregada con miniatura y recolección desactivada.','ok'); } catch(error) { setStatus('scan-status',error.message,'error'); } };
